...
| **Idempotency is bound to the payload, not the key** | A visible stored record holds a SHA-256 digest of the canonical request. A later same-key/same-payload request replays that result; a later different payload returns `409 IDEMPOTENCY_CONFLICT`, so cache replay cannot hand back another document's result. The body is validated before KV. KV cannot atomically reserve the first request: simultaneous first uses may both call AI before either record is visible. |
