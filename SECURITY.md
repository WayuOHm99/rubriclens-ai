...
- The request body is read and validated **before** any cache lookup, so a malformed or differently shaped request cannot read back a cached result. Once a record is visible, reusing its key with a different payload returns `409 IDEMPOTENCY_CONFLICT` rather than another document's result. The first lookup and later write are not an atomic reservation: simultaneous first uses can both reach Gemini, and the last successful KV write wins. This is replay safety, not cross-region single-flight or duplicate-billing prevention.
